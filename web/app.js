...
function recordValueHTML(cell, partId=null) {
  const shown=displayCellValue(cell,partId),record=partId?compositePart(cell,partId):cell;
  if(record?.meaning&&!cell?.error&&cell?.value!=null)return `<span class="coded-value">${esc(record.meaning)}<small>原码 ${esc(record.meaningCode)}</small></span>`;
  return shown==='—'||shown==='未单列'||shown==='待核对'?`<span class="empty-value">${esc(shown)}</span>`:esc(shown);
}
function inspectCell(cell, attrs, className='', partId=null) {
  const shown=displayCellValue(cell,partId),part=partId?compositePart(cell,partId):null,record=partId?part:cell;
  const tooltip=record?.meaning?`${record.meaning} · 原码 ${record.meaningCode} · ${record.meaningSource}`:partId?`${part?.label||'分项记录'}：${shown}${part?.note?' · '+part.note:''}`:cell?.kind==='composite'?`组合写法：${shown} · 查看分项与说明`:`原值：${cell?.error || (cell?.raw ?? cell?.value ?? '空白')}`;
  return `<button type="button" class="cell-button inspect ${cell?.error?'error':''} ${className}" ${attrs} ${partId?`data-component="${esc(partId)}"`:''} title="${esc(tooltip)} · 查看来源与说明">${recordValueHTML(cell,partId)}</button>`;
}
...
function readerExplanation(explanation,{compact=false,title=true}={}) {
  ...
  return `<div class="reader-explanation ${compact?'compact':''}">${title&&explanation.title?`<h3>${esc(explanation.title)}</h3>`:''}${paragraphs.map(p=>`<p>${esc(p)}</p>`).join('')}${explanation.authorNote?`<p>${esc(explanation.authorNote)}</p>`:''}<p class="explanation-source">${esc(reportCitation(explanation))}${explanation.authorNote?` 编码释义：作者确认（${esc(explanation.authorNoteDate)}）。`:''}</p></div>`;
}
...
function renderLibrary() {
  ...
  ].map(([route,title,description,label])=>`<a class="guide-card" href="#/${route}"><p class="eyebrow">${esc(label)}</p><h2>${esc(title)}</h2><p>${esc(description)}</p><span>打开 →</span></a>`).join('')}</div><p class="section-note library-foot">本版本覆盖 ${db.meta.counts.entities} 个实体、${db.meta.counts.sheets} 张工作表。字段数量包含派生与重复记录，不等于独立研究指标数量。单位及其依据可在数值详情中查看。</p>`;
}
...
function renderEntity(id,params=new URLSearchParams()) {
  ...
  const domainHTML=domain=>{
    ...
    return `<details class="detail-section entity-domain-records" id="entity-domain-${esc(domain.id)}" data-detail-key="entity:${esc(entity.id)}:domain:${esc(domain.id)}" ${domain.id===initialDomain?'open':''}>
      ...
      <div class="entity-domain-guide"><p>${esc(guide.intro)}</p>${domain.id==='military'?'<p class="section-note">组合字段分为点前、点后记录。点后未单列时不补零；特殊编码随记录显示释义；核弹头字段的 .1 含义仍待核对。</p>':''}<div class="field-links"><a href="${domainURL(domain.id)}">${esc(domain.name)}领域说明 →</a><a href="${routeURL('compare',{entities:entity.id,metric:domain.metricId})}">比较这个领域 →</a><a href="${routeURL('data',{sheet:domain.id,q:entity.name})}">定位原表 →</a></div></div>
...
function renderMethodology() {
  ...
  <h2>资料来源</h2><p><strong>当前 Excel 提供数值、排名和公式。</strong>网站与下载文件中的研究数值均取自该工作簿，单元格可查看原值、位置和已有公式。</p><p><strong>《综合国力2.0》PDF 提供研究背景和字段解释。</strong>相关说明标注实际 PDF 页码，旧报告数字不覆盖当前 Excel。报告中的“发展”对应当前工作簿的“政治”，“政治”对应当前的“稳定”；网站使用当前工作簿名称。</p><p>单位结合表头、公式和数值量级核对，依据可在数值详情中展开查看。特殊编码采用作者补充的解释。年份和数值来源按原有记录保留。</p><details class="research-note"><summary>背景报告列出的来源</summary><div class="entity-domain-guide">${(m.sources||[]).map(source=>`<div class="source-item"><strong>${source.url&&/^https?:\/\//.test(source.url)?`<a href="${esc(source.url)}" target="_blank" rel="noopener noreferrer">${esc(source.label)} ↗</a>`:esc(source.label)}</strong>${source.note?`<p>${esc(source.note)}</p>`:''}</div>`).join('')}</div></details>
  <h2>阅读数据时</h2><details class="research-note"><summary>数据年份、估算与特殊记录说明</summary><div class="entity-domain-guide"><ul><li>不同字段可能使用不同年份，多数逐项来源、单位及估算标记不完整，横向比较需要结合具体口径。</li><li>若干领域结论和分项名次是已录入的结果。现有材料没有保留底层指标的全部权重、初排及人工调整过程，不能从底层字段完整复现全部研究判断。</li><li>有效国土、军事能力及部分资源分级包含估算或判断，但工作簿没有统一的逐格标记。</li><li>空白、零和 Excel 错误分别保留；同名字段不擅自合并。数据字段包含派生、重复与辅助记录，其数量不等于独立研究指标数。</li><li>军事点号记录按两部分展示。第二项未单列时不补零，可能丢失的尾零不自行恢复；核弹头字段的 .1 含义仍待核对；潜艇与航母的已确认标记随记录显示。</li></ul></div></details><details class="research-note"><summary>本版本完整限制说明</summary><div class="entity-domain-guide">${items(m.limitations)}</div></details>
