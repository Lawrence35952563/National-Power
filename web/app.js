...
const summaryNotice = () => `<p class="reading-note">保留工作簿的既定名次和分类，含小数名次；名次不表示能力倍数。<a href="#/methodology">排名口径 →</a></p>`;
...
function renderFramework() {
  app.innerHTML=breadcrumb(['八个领域'])+mainHeading('','研究说明','八个领域的研究问题、选取依据与相互关系。下列分组用于阅读，不改变原研究的领域或权重。')+`<div class="button-group"><a class="button" href="#/methodology">来源与方法</a><a class="button" href="#/indicators">字段目录</a><a class="button" href="#/downloads">下载文件</a></div>`;
...
function renderRankings(params) {
  ...
  function update(){
    ...
    $('#ranking-reading-note').textContent=expanded?'名次与评分分组呈现，不能直接比较两类数值。':'切换“领域总览”可显示八个领域。';
...
function renderEntity(id,params=new URLSearchParams()) {
  ...
  const structureHTML=kind=>`<section class="entity-structure-section" aria-labelledby="entity-${kind}-heading">
    <div class="entity-structure-heading"><h2 id="entity-${kind}-heading">${kind==='rank'?'三项领域名次':'五项基础评分'}</h2><p class="section-note">${kind==='rank'?'原表顺序；名次不表达能力差距。':'主要使用 1–5 等级；不与名次相加，也不解释为能力倍数。'}</p></div>
...
function renderMethodology() {
  ...
  <h2>现有资料的限制</h2><ul><li>不同字段可能使用不同年份，多数逐项来源、单位及估算标记不完整，横向比较需要结合具体口径。</li><li>若干领域结论和分项名次是已录入的结果。现有材料没有保留底层指标的全部权重、初排及人工调整过程，不能从底层字段完整复现全部研究判断。</li><li>有效国土、军事能力及部分资源分级包含估算或判断，但工作簿没有统一的逐格标记。</li><li>空白、零和 Excel 错误分别保留；同名字段不擅自合并。数据字段包含派生、重复与辅助记录，其数量不等于独立研究指标数。</li><li>军事点号记录按两部分展示。第二项未单列时不补零，可能丢失的尾零不自行恢复；日本部分 .1 等记录的具体含义仍待核对。</li></ul><details class="research-note"><summary>本版本完整限制说明</summary><div class="entity-domain-guide">${items(m.limitations)}</div></details>
