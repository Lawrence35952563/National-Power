...
function renderRankings(params) {
  ...
  function update(){
    ...
    $('#ranking-reading-note').textContent=expanded?'名次与评分分组阅读，领域之间没有统一数值尺度。':'简单榜单突出综合名次与原表分类；选择“领域总览”查看八领域。';
...
function renderEntity(id,params=new URLSearchParams()) {
  ...
  const structureHTML=kind=>`<section class="entity-structure-section" aria-labelledby="entity-${kind}-heading">
    <div class="entity-structure-heading"><h2 id="entity-${kind}-heading">${kind==='rank'?'三项领域名次':'五项基础条件'}</h2><p class="section-note">${kind==='rank'?'原表顺序；名次不表达能力差距。':'主要使用 1–5 等级；不与名次相加，也不解释为能力倍数。'}</p></div>
    <div class="entity-structure-grid ${kind==='score'?'entity-structure-scores':''}">${db.domains.filter(domain=>domain.kind===kind).map(domain=>`<button type="button" class="entity-structure-item" data-open-domain="${esc(domain.id)}" aria-controls="entity-domain-${esc(domain.id)}" aria-expanded="${domain.id===initialDomain?'true':'false'}" title="${esc(guideFor(domain.id).question)}">
      ...
      <span class="entity-structure-action">展开本实体的领域依据 ↓</span>
    </button>`).join('')}</div>
  </section>`;
  ...
  const domainHTML=domain=>{
    ...
    return `<details class="detail-section entity-domain-records" id="entity-domain-${esc(domain.id)}" data-detail-key="entity:${esc(entity.id)}:domain:${esc(domain.id)}" ${domain.id===initialDomain?'open':''}>
      ...
      <div class="entity-domain-guide"><p>${esc(guide.intro)}</p>${domain.id==='military'?'<p class="section-note">组合字段分为点前、点后记录。点后未单列时不补零；日本部分 .1 等记录仍待作者确认，不当作已核实的装备数量。</p>':''}<div class="field-links"><a href="${domainURL(domain.id)}">${esc(domain.name)}为何这样观察 →</a><a href="${routeURL('compare',{entities:entity.id,metric:domain.metricId})}">比较这个领域 →</a><a href="${routeURL('data',{sheet:domain.id,q:entity.name})}">定位原表 →</a></div></div>
      ...
      <div class="entity-domain-footer"><button type="button" class="button text small" data-entity-top>返回结构 ↑</button><a class="button text small" href="${routeURL('data',{sheet:domain.id,q:entity.name})}">定位原表 →</a></div>
    </details>`;
  };

  app.innerHTML=`<div class="entity-page">${breadcrumb(['综合排名','#/rankings'],[entity.name])}
    <header class="entity-hero entity-hero-compact"><div><p class="eyebrow">从位置到结构</p><div class="entity-heading-line"><h1>${esc(entity.name)}</h1><span class="pill">${esc(entity.category||(entity.ranked?'正式排名实体':'补充实体'))}</span></div><p class="entity-en">${esc(entity.englishName||'')}</p></div><div class="entity-overall-rank"><p class="stat-label">综合名次 · 作者原表结果</p><strong>${entity.ranked?esc(format(entity.rank)):'未排名'}</strong><p class="stat-sub">${entity.ranked?'接着看领域结构，理解不同侧面':'补充记录单独阅读'}</p></div></header>
    <div class="entity-primary-action"><a class="button small" href="${routeURL('compare',{entities:entity.id})}">选择其他实体，与${esc(entity.name)}比较 →</a></div>
    ${scopeNote(entity)}
    <section id="entity-structure" class="entity-structure" tabindex="-1" aria-label="${esc(entity.name)}的领域结构"><p class="lead profile-intro">先看政治、经济、军事的相对位置，再看五项基础条件。它们使用不同尺度；点击一个领域，展开${esc(entity.name)}在该领域的具体记录。</p>${['rank','score'].map(structureHTML).join('')}</section>
    <div class="entity-jumpbar" aria-label="领域快速定位"><label for="entity-domain-select">领域定位</label><select class="select" id="entity-domain-select"><option value="">选择领域…</option>${domains.map(domain=>`<option value="${esc(domain.id)}" ${domain.id===initialDomain?'selected':''}>${esc(domain.name)} · ${domainRecords.get(domain.id).populated.length} 项已记录字段</option>`).join('')}</select><button type="button" class="button small" data-entity-top>返回结构 ↑</button></div>
    <section class="entity-evidence"><div class="entity-evidence-heading"><h2>从领域结果追问依据</h2><p class="section-note">每个领域先给出阅读线索，再展示已记录字段。未记录项与辅助列保留在展开项中。</p></div>${domains.map(domainHTML).join('')}</section>
    <details class="research-note entity-reference" id="entity-reference" data-detail-key="entity:${esc(entity.id)}:reference"><summary>计算参考与综合名次的关系</summary><div class="entity-domain-guide"><p>本实体公式参考值：${referenceCell(entity)}。该值不是百分制总分，也不用于重新排列综合名次。</p><a class="button text" href="#/methodology">查看计算方法与作者判断的边界 →</a></div></details>
  </div>`;
...
function renderCompare(params) {
  ...
  app.innerHTML=mainHeading('并列看整体，再追问依据','实体比较','选择 2–5 个实体，在同一张表中查看综合位置与领域结构，再保留几个感兴趣的字段继续比较。')+`
  ...
  function renderDomainDetails() {
    ...
    $('#compare-domain-details').innerHTML=`<details class="research-note compare-domain-disclosure" id="compare-domain-disclosure" ${state.details?'open':''}><summary>八个领域逐项详读 · 展开解释与固定评分刻度</summary><div class="compare-domain-detail-body">${['rank','score'].map(kind=>`<section><h2>${kind==='rank'?'三项领域名次':'五项基础评分'}</h2><p class="section-note">${kind==='rank'?'名次越小，原表位置越靠前；不通过条长暗示能力差距。':'评分采用固定 1–5 等级，不随比较对象变化。小数评分保留原值。'}</p><div class="comparison-grid">${db.domains.filter(domain=>domain.kind===kind).map(domain=>{const indicator=byIndicator.get(domain.metricId);return `<article class="comparison-card"><div class="comparison-card-heading"><h3>${esc(domain.name)}</h3><small>${kind==='rank'?'原表名次':'固定 1–5 等级'}</small></div>${comparisonReading(kind,rowsFor(domain.metricId),indicator)}<p class="comparison-question">${esc(guideFor(domain.id).question)}</p>${readerExplanation(domain.explanation||indicator?.explanation,{compact:true,title:false})}<a class="button text small" href="${domainURL(domain.id)}">理解这个领域 →</a></article>`;}).join('')}</div></section>`).join('')}</div></details>`;
